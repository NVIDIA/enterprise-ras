...
This tests API connectivity, authentication, budget, and SSH key registration. The `ARCH` parameter is used to load the Air URL from the per-deployment inventory; credentials themselves come from the shared vault, so this works even before any inventory exists.
...
4. Note the external hostname and port (e.g., `worker34.air-inside.nvidia.com:26788`)
...
2. Go to the **Services** tab → **Add Service**
3. Service Type: **SSH**, Internal Port: **22**
4. Note the external hostname and port (e.g., `worker34.air-inside.nvidia.com:18252`)

### 3.3 Verify SSH Access

```bash
ssh -p 26788 ubuntu@worker34.air-inside.nvidia.com   # oob-server-01
ssh -p 18252 ubuntu@worker34.air-inside.nvidia.com   # dhcp-oob
```
...
```yaml
ansible_host: worker34.air-inside.nvidia.com
ansible_port: 26788
ansible_user: ubuntu
...
```
ssh -p <port> ubuntu@<worker>.air-inside.nvidia.com
```
...
**Symptom**: `make deploy-servers` times out or prompts for `ubuntu@...air-inside.nvidia.com's password`
