...
# NVUE CLI Configuration for gl-plane1-15
...
nv set interface lo ipv4 address 10.1.1.15/32
...
nv set interface swp1s0,swp1s1,swp2s0,swp2s1,swp3s0,swp3s1,swp4s0,swp4s1,swp5s0,swp5s1,swp6s0,swp6s1,swp7s0,swp7s1,swp8s0,swp8s1,swp9s0,swp9s1,swp10s0,swp10s1,swp11s0,swp11s1,swp12s0,swp12s1,swp13s0,swp13s1,swp14s0,swp14s1,swp15s0,swp15s1,swp16s0,swp16s1,swp17s0,swp17s1,swp18s0,swp18s1,swp19s0,swp19s1,swp20s0,swp20s1,swp21s0,swp21s1,swp22s0,swp22s1,swp23s0,swp23s1,swp24s0,swp24s1,swp25s0,swp25s1,swp26s0,swp26s1,swp27s0,swp27s1,swp28s0,swp28s1,swp29s0,swp29s1,swp30s0,swp30s1,swp31s0,swp31s1,swp32s0,swp32s1 bridge domain br_default access 900

# vlan900 SVI (GPU VRF + VRR)
# 5.16 NVUE: SVI uses ipv4/ipv6 sub-objects; vrf is set without "ip" prefix.
nv set interface vlan900 ipv4 address 192.168.0.16/20
...
nv set nve vxlan source address 10.1.1.15
...
# Router policy
#============================================================================
nv set router bgp autonomous-system 4260395902
nv set router bgp state enabled
nv set router bgp router-id 10.1.1.15
...
nv set system hostname gl-plane1-15
...
#       You are successfully logged in to: gl-plane1-15 - site: largescale / arch: 2-8-9-400-SP
####################################################################'
...
nv set vrf GPU loopback ip address 10.1.1.25/32
...
nv set vrf GPU router bgp autonomous-system 4260395902
nv set vrf GPU router bgp state enabled
nv set vrf GPU router bgp router-id 10.1.1.15
...
nv set vrf default router bgp autonomous-system 4260395902
nv set vrf default router bgp state enabled

# Overlay peers (numbered, loopback-to-loopback)
nv set vrf default router bgp neighbor 10.1.1.41 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.41 type numbered
nv set vrf default router bgp neighbor 10.1.1.42 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.42 type numbered
nv set vrf default router bgp neighbor 10.1.1.43 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.43 type numbered
nv set vrf default router bgp neighbor 10.1.1.44 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.44 type numbered
nv set vrf default router bgp neighbor 10.1.1.45 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.45 type numbered
nv set vrf default router bgp neighbor 10.1.1.46 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.46 type numbered
nv set vrf default router bgp neighbor 10.1.1.47 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.47 type numbered
nv set vrf default router bgp neighbor 10.1.1.48 peer-group overlay
nv set vrf default router bgp neighbor 10.1.1.48 type numbered
...
nv set vrf default router bgp router-id 10.1.1.15
