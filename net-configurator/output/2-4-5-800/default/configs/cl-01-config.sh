...
nv set router policy prefix-list INBAND_LOCAL_IF rule 10 match 172.16.1.41/32 max-prefix-len 32
...
nv set router policy prefix-list INBAND_PREFIXES rule 30 match 172.16.1.41/32 max-prefix-len 32
...
nv set router policy prefix-list LOCAL_OOB_LOOPBACK rule 10 match 172.16.1.31/32 max-prefix-len 32
nv set router policy prefix-list OOB_LOCAL_IF rule 10 action permit
nv set router policy prefix-list OOB_LOCAL_IF rule 10 match 172.16.1.31/32 max-prefix-len 32
...
nv set router policy prefix-list OOB_PREFIXES rule 20 match 172.16.1.31/32 max-prefix-len 32
...
nv set vrf INBAND loopback ip address 172.16.1.41/32
...
nv set vrf INBAND router bgp router-id 172.16.1.41

nv set vrf OOB evpn state enabled
nv set vrf OOB evpn vlan 3001
nv set vrf OOB evpn vni 5001

nv set vrf OOB loopback ip address 172.16.1.31/32
...
nv set vrf OOB router bgp router-id 172.16.1.31
...
nv set vrf STORAGE loopback ip address 172.16.1.51/32
...
nv set vrf STORAGE router bgp router-id 172.16.1.51
...
#       You are successfully logged in to: cl-01 - site: default / arch: 2-4-5-800
####################################################################'
