...
# NVUE CLI Configuration for gl-plane2-12
...
nv set interface lo ipv4 address 10.1.2.12/32
...
nv set interface vlan900 ipv4 address 192.168.16.13/20
...
nv set nve vxlan source address 10.1.2.12
...
# Router policy
#============================================================================
nv set router bgp autonomous-system 4260396899
nv set router bgp state enabled
nv set router bgp router-id 10.1.2.12
...
nv set system hostname gl-plane2-12
...
#       You are successfully logged in to: gl-plane2-12 - site: largescale / arch: 2-8-9-800
####################################################################'
...
nv set vrf GPU loopback ip address 10.1.2.22/32
...
nv set vrf GPU router bgp autonomous-system 4260396899
nv set vrf GPU router bgp state enabled
nv set vrf GPU router bgp router-id 10.1.2.12
...
nv set vrf default router bgp autonomous-system 4260396899
...
nv set vrf default router bgp neighbor 10.1.2.47 peer-group overlay
nv set vrf default router bgp neighbor 10.1.2.47 type numbered
nv set vrf default router bgp neighbor 10.1.2.48 peer-group overlay
nv set vrf default router bgp neighbor 10.1.2.48 type numbered
...
nv set vrf default router bgp router-id 10.1.2.12
